...
import { createOrder, getVoucherByCode, createMoMoPayment } from '@/api';
...
import { getWalletBalance, useWalletForPayment } from '@/api/wallet';
...
import { getPaymentConfig, type PaymentConfig } from '@/api/paymentConfig';
...
// ✅ qtyOf luôn ưu tiên item[0].quantity (số lượng thực của linh kiện trong bundle)
const qtyOf = (e: RawCartEntry) => {
  if (Array.isArray(e.item) && e.item[0] && typeof e.item[0].quantity === 'number') {
    return Math.max(1, e.item[0].quantity);
  }
  if (typeof (e as any).accessoryQuantity === 'number' && (e as any).accessoryQuantity > 0) {
    return (e as any).accessoryQuantity;
  }
  if (
    typeof (e as any).terrariumVariantQuantity === 'number' &&
    (e as any).terrariumVariantQuantity > 0
  ) {
    return (e as any).terrariumVariantQuantity;
  }
  if (typeof e.totalCartQuantity === 'number' && e.totalCartQuantity > 0) {
    return e.totalCartQuantity;
  }
  return 1;
};
...
// ===== Small logger =====
const isDev =
  typeof import.meta !== 'undefined' ? import.meta.env.MODE !== 'production' : true;
const log = {
  group(title: string) {
    if (!isDev) return;
    try {
      console.groupCollapsed(`%c${title}`, 'color:#16a34a;font-weight:600;');
    } catch {}
  },
  end() {
    if (!isDev) return;
...
// ✅ unified image onError handler (chuẩn TS)
...
const Checkout: React.FC = () => {
  ...
  const [comboMeta, setComboMeta] = useState<
    Record<number, { name: string; image: string; items: any[] }>
  >({});
  ...
  (async () => {
    try {
      ...
      console.error('Không load được PaymentConfig:', err);
    }
  })();
}, []);
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const b of apiCart.bundleItems || [])
        for (const e of b.bundleAccessories || []) e.accessoryId && ids.add(e.accessoryId);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      (apiCart?.bundleItems || []).forEach((b) => {
        ...
        (b.bundleAccessories || []).forEach(
          (ba) => ba.terrariumVariantId && variantIds.add(ba.terrariumVariantId),
        );
      });
      ...
      if (Object.keys(newMap).length)
        setVariantToTerrariumMap((prev) => ({ ...prev, ...newMap }));
      ...
      if (terrariumIdsToFetch.size) {
        const needMeta = [...terrariumIdsToFetch].filter(
          (tid) => !terrariumName[tid] || !terrariumThumb[tid],
        );
        if (needMeta.length) {
  ...
  useEffect(() => {
    const fetchComboMeta = async () => {
      ...
      const missingIds = [...new Set(combosSelected.map((e) => e.comboId!))].filter(
        (id) => !comboMeta[id],
      );
      if (!missingIds.length) return;

      const results = await Promise.all(
        missingIds.map(async (comboId) => {
          try {
                ...
                if (item.accessoryId) {
                  ...
                  try {
                    ...
                    return {
                      ...
                      image:
                        variant?.urlImage ||
                        terr?.terrariumImages?.[0]?.imageUrl ||
                        FALLBACK_IMG,
                      type: 'terrarium',
  ...
  const voucherApplicable = useMemo(() => {
    ...
    const now = new Date();
    const validDate =
      new Date(voucher.validFrom) <= now && now <= new Date(voucher.validTo);
  ...
  // nếu orderAmount = 0 nghĩa là freeship mọi đơn
  if (paymentConfig.orderAmount === 0) return 0;

  // nếu freeshipAmount = 0 nghĩa là không áp dụng freeship
  if (paymentConfig.freeshipAmount === 0) return 0;

  return subtotal >= paymentConfig.orderAmount
    ? 0
    : paymentConfig.freeshipAmount;
}, [subtotal, paymentConfig]);



  const discountFromFull = useMemo(() => {
  if (paymentOption !== 'full' || !paymentConfig) return 0;
  if (!paymentConfig.fullPaymentDiscountPercent) return 0; // 0 hoặc null

  const base = Math.max(0, subtotal - discountFromVoucher + shippingFee);
  return Math.floor((base * paymentConfig.fullPaymentDiscountPercent) / 100);
}, [paymentOption, subtotal, discountFromVoucher, shippingFee, paymentConfig]);



  // Totals
  const totalAmountOld = Math.max(0, subtotal + shippingFee);
  const totalAmountNew = Math.max(
    0,
    subtotal - discountFromVoucher + shippingFee - discountFromFull,
  );
 ...
 const actualPaymentAmount = useMemo(() => {
  ...
  if (paymentOption === 'deposit') {
    if (!paymentConfig.depositPercent) return 0; // nếu =0 nghĩa là không cho đặt cọc
    return Math.round(
      (Math.max(0, subtotal - discountFromVoucher + shippingFee) *
        paymentConfig.depositPercent) / 100
    );
  }

  return Math.round(totalAmountNew);
}, [paymentOption, subtotal, discountFromVoucher, shippingFee, totalAmountNew, paymentConfig]);
  ...
  const applyVoucher = async () => {
    ...
    try {
      ...
      if ((res.minOrderAmount ?? 0) > subtotal) {
        setVoucher(res);
        setVoucherError(
          `Áp dụng cho đơn từ ${(res.minOrderAmount || 0).toLocaleString('vi-VN')} VND (tạm tính chưa đạt).`,
        );
  ...
  const buildOrderItems = (): CreateOrderItemWithCombo[] => {
    ...
    for (const b of bundlesAll) {
      ...
      const variantFromAnyAcc =
        (b?.bundleAccessories || []).find((x) => !!x.terrariumVariantId)?.terrariumVariantId ??
        0;
      ...
      if (!bundleVariantId) {
        ...
        toast.error(
          'Không xác định được biến thể (variant) cho một bộ phụ kiện. Vui lòng mở lại trang/ chọn đúng biến thể.',
        );
        continue;
      }
      ...
      for (const e of b.bundleAccessories) {
        ...
        // ✅ LUÔN dùng qtyOf(e) để lấy item[0].quantity nếu có
        const qty = qtyOf(e);
    ...
    for (const e of mergedSingles) {
      const qty = qtyOf(e); // ✅ ưu tiên quantity đúng
  ...
  // Cleanup cart
  // Cleanup cart (xoá đúng toàn bộ bundle + mainItem)
const cleanupCartItems = async (orderItems: CreateOrderItemWithCombo[], selectedCombos: RawCartEntry[]) => {
  try {
    ...
    // ----- Build index -----
    // key cho accessory trong bundle: `${accessoryId}|${variantId}`
    const accInBundleKeyToId: Record<string, number> = {};
    // mainItem theo variant (để xoá vỏ bundle)
    const bundleMainByVariant: Record<number, number> = {};
    // single accessory (không thuộc bundle)
    const singleAccIdToCartId: Record<number, number> = {};
    // single/main variant (không thuộc bundle)
    const singleVariantToCartId: Record<number, number> = {};
    // combo
    const comboIdToCartId: Record<number, number> = {};

    for (const b of bundles) {
      const main = b?.mainItem;
      if (main?.cartItemId && main?.terrariumVariantId) {
        bundleMainByVariant[main.terrariumVariantId] = main.cartItemId;
      }
      for (const e of (b?.bundleAccessories || [])) {
        if (e?.cartItemId && e?.accessoryId) {
          const key = `${e.accessoryId}|${e.terrariumVariantId || 0}`;
          accInBundleKeyToId[key] = e.cartItemId;
        }
      }
      // các bundle không có accessory (biến thể bán lẻ) – ghi index theo variant
      if ((b?.bundleAccessories?.length || 0) === 0 && main?.terrariumVariantId && main?.cartItemId) {
        singleVariantToCartId[main.terrariumVariantId] = main.cartItemId;
      }
    }

    for (const s of singles) {
      if (s?.comboId && s?.cartItemId) comboIdToCartId[s.comboId] = s.cartItemId;
      else if (s?.accessoryId && s?.cartItemId) singleAccIdToCartId[s.accessoryId] = s.cartItemId;
      else if (s?.terrariumVariantId && s?.cartItemId) singleVariantToCartId[s.terrariumVariantId] = s.cartItemId;
    }

    // ----- Tập cần xoá -----
    const toDelete = new Set<number>();
    const touchedBundleVariants = new Set<number>(); // để xoá kèm mainItem

    for (const it of orderItems) {
      // 1) BUNDLE_ACCESSORY: ưu tiên map theo accessoryId + variant
      if (it.itemType === 'BUNDLE_ACCESSORY' && it.accessoryId && it.terrariumVariantId) {
        const key = `${it.accessoryId}|${it.terrariumVariantId}`;
        const id = accInBundleKeyToId[key];
        if (id) {
          toDelete.add(id);
          touchedBundleVariants.add(it.terrariumVariantId);
        }
        continue;
      }

      // 2) MAIN_ITEM (không thuộc bundle) – theo variant
      if (it.itemType === 'MAIN_ITEM' && it.terrariumVariantId) {
        const id = singleVariantToCartId[it.terrariumVariantId];
        if (id) toDelete.add(id);
        continue;
      }

      // 3) SINGLE accessory (không thuộc bundle) – theo accessoryId
      if (it.itemType === 'SINGLE' && it.accessoryId) {
        const id = singleAccIdToCartId[it.accessoryId];
        if (id) toDelete.add(id);
        continue;
      }

      // 4) COMBO – theo comboId
      if ((it as any).itemType === 'COMBO' && (it as any).comboId) {
        const id = comboIdToCartId[(it as any).comboId as number];
        if (id) toDelete.add(id);
        continue;
      }
    }

    // ----- Xoá luôn mainItem của các bundle đã đụng tới -----
    for (const vid of touchedBundleVariants) {
      const mainId = bundleMainByVariant[vid];
      if (mainId) toDelete.add(mainId);
    }

    if (toDelete.size > 0) {
      await Promise.all([...toDelete].map((cartItemId) => deleteCartItem(cartItemId)));
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        isPayFull: paymentOption === 'full',
      } as const;

      // 🔎 Debug
      log.group('Checkout ▶ CREATE ORDER PAYLOAD');
      log.info('Payload gửi đi:');
      console.log(payload);
      log.info('Danh sách items:');
      console.table(
        items.map((i) => ({
          itemType: i.itemType,
          accessoryId: i.accessoryId,
          accessoryQuantity: i.accessoryQuantity,
          terrariumVariantId: i.terrariumVariantId,
          terrariumVariantQuantity: i.terrariumVariantQuantity,
          comboId: (i as any).comboId,
          comboQuantity: (i as any).comboQuantity,
        })),
      );
      log.end();

      localStorage.setItem('lastCreateOrderPayload', JSON.stringify(payload, null, 2));

      const isDebugOrder =
        typeof window !== 'undefined' &&
        new URLSearchParams(window.location.search).get('debugOrder') === '1';
      if (isDebugOrder) {
        toast.info('DEBUG MODE: Đã log payload và lưu localStorage, không gọi API.');
        return;
      }
      ...
      try {
        await sendWebNotification(
          ...
          `Đơn hàng #${orderId} đã được tạo. Tổng tiền: ${totalAmountNew.toLocaleString(
            'vi-VN',
          )} VND`,
        );
      } catch (e) {
  ...
  /* ======================= UI: SẢN PHẨM / ĐỊA CHỈ / VOUCHER / SUMMARY ======================= */
  const ProductSectionAPI = (
    <div className="bg-white p-4 sm:p-5 rounded-lg shadow space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="font-semibold">
                  ...
                  <div className="text-sm text-gray-600">
                    SL: <b>{e.totalCartQuantity || qtyOf(e)}</b>
                  </div>
                </div>
              </div>
      ...
      {bundlesAll.map((b) => {
        ...
        const name = tid
          ? (terrariumName[tid] || `Bộ terrarium #${tid}`)
          : 'Bộ phụ kiện theo biến thể';
            ...
            {isOpen && (
              <div className="divide-y">
                {b.bundleAccessories.map((e) => {
                  ...
                  const aName = aId
                    ? accessoryName[aId] || e.item?.[0]?.productName || `Phụ kiện #${aId}`
                    : 'Phụ kiện';
                  const aThumb = aId
                    ? accessoryThumb[aId] || e.item?.[0]?.imageUrl || FALLBACK_IMG
                    : FALLBACK_IMG;
          ...
          <div className="lg:col-span-2 space-y-4 sm:space-y-6">
            ...
            <div className="relative">
              ...
              <div
                ...
                title="Hiện chỉ hỗ trợ giao nội thành TP.HCM"
              >
                TP.HCM nội thành
              </div>
            </div>
            ...
            <div className="bg-white p-4 sm:p-5 rounded-lg shadow">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                ...
                >
                  <label className="flex items-center cursor-pointer">
                    ...
                    <span className="font-bold text-yellow-700 text-sm sm:text-base md:text-lg">
                        Cọc trước {paymentConfig?.depositPercent
                                  ? `Cọc trước ${paymentConfig.depositPercent}%`
                  ...
                  <div className="mt-2 text-xs sm:text-sm text-gray-700">
                      Đặt cọc {paymentConfig?.depositPercent ?? 30}% để đảm bảo đơn hàng; phí ship được tính kèm lần thanh toán này.
                    </div>
                ...
                >
                  ...
                  <div className="mt-2 text-xs sm:text-sm text-gray-700">
                    <b>{paymentConfig?.fullPaymentDiscountPercent
                    ? `Giảm ngay ${paymentConfig.fullPaymentDiscountPercent}%`
                    : 'Không giảm thêm khi thanh toán toàn bộ'}
                  </b> giá trị sản phẩm sau khi áp voucher.
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 sm:gap-6 w-full">
              <div className="flex-1 bg-white rounded-lg shadow p-4 sm:p-5">
                ...
                <div className="mt-4 min-h-[80px]">
                  {voucherError && (
                    <div className="text-red-500 text-sm sm:text-base">{voucherError}</div>
                  )}
                  {voucher && (
                    <div className="border-2 border-green-600 rounded p-4 bg-green-50 text-green-700 text-sm sm:text-base font-medium space-y-1 mt-1">
                      ...
                      {voucher.minOrderAmount ? (
                        <div>
                          Áp dụng cho đơn từ{' '}
                          <b>{voucher.minOrderAmount.toLocaleString('vi-VN')} VND</b>
                        </div>
                      ) : null}
                      <div>
                        {voucher.discountPercent && voucher.discountPercent > 0 ? (
                          ...
                          <>
                            Giảm:{' '}
                            <b>
                              {(voucher.discountAmount || 0).toLocaleString('vi-VN')} VND
                            </b>
                          </>
                        )}
                      </div>
                ...
                <div className="flex justify-between items-end mb-3">
                  ...
                  <span
                    className={`text-xs sm:text-sm ${
                      customerNote.trim().split(/\s+/).filter(Boolean).length > 100
                        ? 'text-red-500'
                        : 'text-gray-500'
                    }`}
                  >
            ...
            <div className="lg:col-span-1 space-y-4 sm:space-y-6">
              ...
              <div className="bg-white border border-gray-200 rounded-lg shadow p-4 sm:p-5 sticky top-4 sm:top-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-base sm:text-lg md:text-xl font-bold text-green-700">
                    Tổng kết đơn hàng
                  </h2>
                  <button
                ...
                <div className="space-y-2 text-sm sm:text-base">
                  ...
                  {voucher && (
                    <div
                      className={`flex justify-between ${
                        voucherApplicable ? 'text-yellow-600' : 'text-gray-400'
                      }`}
                    >
                  ...
                  {/* 👉 Cụm mới hiển thị điều kiện freeship */}
                  {paymentConfig && (
                    <div className="ml-2 mt-1 text-xs text-gray-600 italic">
                      {paymentConfig.orderAmount === 0 ? (
                        <span className="text-green-600">
                          Tất cả đơn hàng đều được miễn phí ship.
                        </span>
                      ) : paymentConfig.freeshipAmount === 0 ? (
                        <span className="text-gray-500">
                          Không áp dụng freeship cho đơn hàng.
                        </span>
                      ) : subtotal >= paymentConfig.orderAmount ? (
                        <span className="text-green-600">
                          Bạn đã đủ điều kiện miễn phí ship cho đơn từ{' '}
                          {currency(paymentConfig.orderAmount)}.
                        </span>
                      ) : (
                        <span>
                          Đơn từ {currency(paymentConfig.orderAmount)} sẽ được miễn phí ship
                          (phí hiện tại: {currency(paymentConfig.freeshipAmount)}).
                        </span>
                      )}
                    </div>
                  )}
                  ...
                  {paymentOption === 'deposit' && (
                    <div className="flex justify-between text-red-600">
                      ...
                      <span>
                        {(totalAmountNew - actualPaymentAmount).toLocaleString('vi-VN')} VND
                      </span>
                    </div>
                  )}
                </div>
