...
const ComboEdit: React.FC = () => {
  ...
  const fetchComboData = async () => {
    if (!id) {
      notification.error({
        message: 'Lỗi',
        description: 'Không tìm thấy ID combo.',
        placement: 'topRight',
      });
    ...
    try {
      ...
      if (result.status === 200 && result.data) {
        const comboData: ComboData = result.data;

        // Transform combo items to form data format
        const transformedItems = comboData.items.map(item => ({
        ...
        setFormData({
          ...
          comboCategoryId: comboData.comboCategoryId,
          name: comboData.name,
          description: comboData.description,
          imageUrl: comboData.imageUrl,
          comboPrice: comboData.comboPrice,
          discountPercent: comboData.discountPercent,
          stockQuantity: comboData.stockQuantity,
          isFeatured: comboData.isFeatured,
          items: transformedItems,
        });
      } else {
        throw new Error(result.message || 'Failed to fetch combo data');
      }
    } catch (error) {
      console.error('Error fetching combo data:', error);
      notification.error({
        message: 'Lỗi',
        description: `Không thể tải thông tin combo: ${error instanceof Error ? error.message : 'Unknown error'}`,
        placement: 'topRight',
      });
      navigate('/manager/combo/list');
    } finally {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // After fetching all reference data, fetch combo data
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Chỉnh sửa Combo</h1>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <span>{loading ? 'Đang cập nhật...' : 'Cập nhật Combo'}</span>
                </button>
                <button
                  type="button"
                  onClick={() => navigate('/manager/combo/list')}
